...
public class ItemFlyingCarpet extends Item {
	...
	private final String name = "flying_carpet";
        ...
        else
        {
            ...
            else
            {
                if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
                {
                    ...
                    float f10 = (float)(((MathHelper.floor_double((double)(playerIn.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90)+90f;
                    EntityFlyingCarpet entityFlyingCarpet = new EntityFlyingCarpet(worldIn, (double)((float)blockpos.getX() + 0.5F), (double)((float)blockpos.getY() + 1.0F), (double)((float)blockpos.getZ() + 0.5F), f10);

                    ItemStack clone = itemStackIn.copy();
                    clone.stackSize = 1;
                    if (!worldIn.getCollidingBoundingBoxes(entityFlyingCarpet, entityFlyingCarpet.getEntityBoundingBox().expand(-0.1D, -0.1D, -0.1D)).isEmpty())
                    {
                        return itemStackIn;
                    }
                    ...
                    if (!worldIn.isRemote)
                    {
                    	entityFlyingCarpet.setEntityItemStack(clone);
                        worldIn.spawnEntityInWorld(entityFlyingCarpet);
                    }
