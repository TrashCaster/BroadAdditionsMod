...
public class ContainerBackpack extends Container {
	 ...
	 */
	private static final int ARMOR_START = InventoryBackpack.SLOT_COUNT, ARMOR_END = ARMOR_START + 3,
			INV_START = ARMOR_END + 1, INV_END = INV_START + 26, HOTBAR_START = INV_END + 1,
			HOTBAR_END = HOTBAR_START + 8;

	private final EntityPlayer player;
	private final EntityBackpack backpack;

	public ContainerBackpack(EntityPlayer player, EntityBackpack backpack, InventoryPlayer playerInventory, InventoryBackpack backpackInv) {
		this.player = player;
		...
		for (i = 0; i < InventoryBackpack.SLOT_COUNT; i++) {
			this.addSlotToContainer(
					new Slot(backpackInv, i, 88 + 18 * (i % 3), 26 + (18 * (int) Math.floor((float) i / 3f))) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (par2 < INV_START) {
				...
				if (itemstack1.isStackable()) {
					if (!this.mergeItemStack(itemstack1, 0, InventoryBackpack.SLOT_COUNT, false)) {
						return null;
					}
		...
		if (stack.isStackable()) {
			while (stack.stackSize > 0 && (!reverseDirection && i < endIndex || reverseDirection && i >= startIndex)) {
				...
				if (itemstack != null && itemstack.getItem() == stack.getItem()
					...
					System.out.println("Slot "+i+" limit is "+k);
					if (j <= k) {
						stack.stackSize = 0;
